...
def predict():
    ...
    predict_dataset = TensorDataset(
        ...
        predict_processor.labels_tensor  # 这里仍然需要labels_tensor作为占位符
    )
    ...
    _, predictions, _ = evaluate_model(model, predict_loader, device)
    
    # Load the original prediction data file
    df = pd.read_excel(DATA_CONFIG['file_path'] + DATA_CONFIG['predict_dataset_path'])
    
    # Add predictions to the DataFrame
    df['predict'] = predictions.flatten()
    
    # Save the updated DataFrame
    df.to_excel(DATA_CONFIG['file_path'] + 'predicted_' + DATA_CONFIG['predict_dataset_path'], index=False)
    print(f"Updated predictions saved to predicted_{DATA_CONFIG['predict_dataset_path']}")
